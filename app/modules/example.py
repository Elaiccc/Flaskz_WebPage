...
class DepartmentDeviceModel(ModelBase, ModelMixin):
    __tablename__ = 'ex_department_devices'  # 表名

    id = Column(Integer, primary_key=True, autoincrement=True)
    name = Column(String(64), nullable=False)  # 设备名称
    ip_address = Column(String(64))  # IP 地址
    location = Column(String(255))  # 安装位置

    department_id = Column(Integer, ForeignKey('ex_departments.id'), nullable=False)
    department = relationship('DepartmentModel', backref='devices')

    created_at = Column(DateTime(), default=datetime.now)
    updated_at = Column(DateTime(), default=datetime.now, onupdate=datetime.now)

    like_columns = ['name', 'location']
